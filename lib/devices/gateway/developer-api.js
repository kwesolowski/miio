		...
		this.socket.on('listening', () => {
			...
			for(const name of Object.keys(interfaces)) {
				...
				for(const addr of addresses) {
					if(addr.family === 'IPv4') {
						try {
							this.debug("Adding multicast ", MULTICAST_ADDRESS ," membership for", addr.address);
							this.socket.addMembership(MULTICAST_ADDRESS, addr.address);
						} catch(ex) {
							console.warn("Adding multicast ", MULTICAST_ADDRESS ," membership for", addr.address, "failed with error", ex)
						}
					}
				}
			}
